/*
 * License: MIT
 * Copyright (c)  Pela Cristian 2018.
 */

package com.crskdev.biblereaderplus.presentation.read

/**
 * Created by Cristian Pela on 01.11.2018.
 */

val MOCKED_BIBLE_DATA_SOURCE = listOf<ReadUI>(
    ReadUI.BookUI(0, "Genesis"),
    ReadUI.ChapterUI(0, 0, "Chapter 1"),
    ReadUI.VersetUI(1, 0, 0, 1, "In the beginning God created the heavens and the earth."),
    ReadUI.VersetUI(
        2,
        0,
        0,
        2,
        "Now the earth was formless and empty, darkness was over the surface of the deep, and the Spirit of God was hovering over the waters"
    ),
    ReadUI.VersetUI(3, 0, 0, 3, "And God said, “Let there be light,” and there was light."),
    ReadUI.VersetUI(
        4,
        0,
        0,
        4,
        "God saw that the light was good, and he separated the light from the darkness."
    ),
    ReadUI.VersetUI(
        5,
        0,
        0,
        5,
        "God called the light “day,” and the darkness he called “night.” And there was evening, and there was morning—the first day"
    ),
    ReadUI.VersetUI(
        6,
        0,
        0,
        6,
        "And God said, “Let there be a vault between the waters to separate water from water.”"
    ),
    ReadUI.VersetUI(
        7,
        0,
        0,
        7,
        "So God made the vault and separated the water under the vault from the water above it. And it was so. "
    ),
    ReadUI.VersetUI(
        8,
        0,
        0,
        8,
        "God called the vault “sky.” And there was evening, and there was morning—the second day"
    ),
    ReadUI.VersetUI(
        9,
        0,
        0,
        9,
        "And God said, “Let the water under the sky be gathered to one place, and let dry ground appear.” And it was so. "
    ),
    ReadUI.VersetUI(
        10,
        0,
        0,
        10,
        "God called the dry ground “land,” and the gathered waters he called “seas.” And God saw that it was good."
    ),
    ReadUI.ChapterUI(1, 0, "Chapter 2"),
    ReadUI.VersetUI(
        11,
        0,
        1,
        1,
        "Thus the heavens and the earth were completed in all their vast array."
    ),
    ReadUI.VersetUI(
        12,
        0,
        1,
        2,
        "By the seventh day God had finished the work he had been doing; so on the seventh day he rested from all his work."
    ),
    ReadUI.VersetUI(
        13,
        0,
        1,
        3,
        "Then God blessed the seventh day and made it holy, because on it he rested from all the work of creating that he had done."
    ),
    ReadUI.VersetUI(
        14,
        0,
        1,
        4,
        "This is the account of the heavens and the earth when they were created, when the Lord God made the earth and the heavens."
    ),
    ReadUI.VersetUI(
        15,
        0,
        1,
        5,
        "Now no shrub had yet appeared on the earth[a] and no plant had yet sprung up, for the Lord God had not sent rain on the earth and there was no one to work the ground,"
    ),
    ReadUI.VersetUI(
        16,
        0,
        1,
        6,
        "but streams[b] came up from the earth and watered the whole surface of the ground. "
    ),
    ReadUI.VersetUI(
        17,
        0,
        1,
        7,
        "Then the Lord God formed a man[c] from the dust of the ground and breathed into his nostrils the breath of life, and the man became a living being."
    ),
    ReadUI.VersetUI(
        18,
        0,
        1,
        8,
        "Now the Lord God had planted a garden in the east, in Eden; and there he put the man he had formed."
    ),
    ReadUI.BookUI(1, "Exodus"),
    ReadUI.ChapterUI(2, 1, "Chapter 1"),
    ReadUI.VersetUI(
        1,
        1,
        2,
        1,
        "These are the names of the sons of Israel who went to Egypt with Jacob, each with his family: 2 Reuben, Simeon, Levi and Judah;"
    ),
    ReadUI.VersetUI(1, 1, 2, 2, "Issachar, Zebulun and Benjamin;"),
    ReadUI.VersetUI(1, 1, 2, 3, "Reuben, Simeon, Levi and Judah;"),
    ReadUI.VersetUI(1, 1, 2, 4, "Dan and Naphtali; Gad and Asher."),
    ReadUI.VersetUI(
        1,
        1,
        2,
        5,
        "The descendants of Jacob numbered seventy[a] in all; Joseph was already in Egypt."
    ),
    ReadUI.VersetUI(1, 1, 2, 6, "Now Joseph and all his brothers and all that generation died, "),
    ReadUI.VersetUI(
        1,
        1,
        2,
        7,
        "but the Israelites were exceedingly fruitful; they multiplied greatly, increased in numbers and became so numerous that the land was filled with them."
    ),
    ReadUI.VersetUI(
        1,
        1,
        2,
        8,
        "Then a new king, to whom Joseph meant nothing, came to power in Egypt."
    )
)